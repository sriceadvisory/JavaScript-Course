...
document.querySelector('.check').addEventListener('click', function () {
    ...
    if (!guess) {
        displayMessage('⛔ No number!'); //change the message on button click

        //The player guesses the correct number
    } else if (guess === secretNumber) {
        displayMessage('🎉 Correct Number!'); //change the message on button click
        ...
        if (score >= 1) {
            displayMessage(guess > secretNumber ? '📈 Too high!' : '📉 Too Low'); //change the message on button click
